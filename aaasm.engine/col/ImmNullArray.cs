...
namespace aaasm.engine.col
{
    ...
    /// </summary>
    public readonly struct ImmNullArray<T> : IReadOnlyList<T>
    {
        #region init

        /// <summary>Initializer for <see cref="ImmNullArray{T}"/></summary>
        ...
        /// </exception>
        public ImmNullArray(IReadOnlyList<T> elements)
        {
            try
            {
        ...
        ///     <paramref name="elements"/> contains one or more null elements
        /// </exception>
        public ImmNullArray(IEnumerable<T> elements)
        {
            IEnumerable<T> loop()
            {
        ...
        public static ImmNullArray<T> EMPTY { get; } = new();
